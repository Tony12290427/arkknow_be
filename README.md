...
**ABSOLUTE RED LINE — never commit:**

| Category | Examples |
|----------|----------|
| Cloud AccessKeys | `LTAI...`, AccessKey Secret |
| API Keys | `sk-proj-...`, `sk-...`, OpenAI/DeepSeek keys |
| Database passwords | `MYSQL_ROOT_PASSWORD` values |
| JWT private keys | `private.pem`, RSA private key |
| Server IPs | `8.218.64.22` (use `<server-ip>` in docs) |
| SSH keys | `.pem` files, private keys |
| Personal passwords | Any real credentials |

**Commit message rules:**
- Never write "remove key", "fix leak", "cleanup secrets"
- Use neutral language: `chore: rotate credentials`, `chore: update config`

**If secrets were committed:** Use `git filter-branch` to scrub entire history (not `git rm` which leaves history intact), then force push.
